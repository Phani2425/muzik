...
export const getQueue = async (roomId: string) => {
  ...
  for (let i = 0; i < trackList.length; i += 2) {
    ...
    const voteCount = Math.round(score / multiplier);
    const position = i / 2;
...
export const putCurrTrackToTop = async (playerState:PlyerSate) => {
  // Increase the vote of the newly current track to 999 so it always stays at top of queue
  const score = await redis.zscore(
    `room:${playerState.roomId}`,
    playerState.currTrack
  );

  if (score) {
    const intScore = parseInt(score);
    const timestamp = intScore % multiplier;
    const newScore = 999 * multiplier + timestamp;
    await redis.zadd(
      `room:${playerState.roomId}`,
      newScore,
      playerState.currTrack
